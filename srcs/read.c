/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   read.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: edechena <edechena@student.42lyon.fr>      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/24 10:10:53 by edechena          #+#    #+#             */
/*   Updated: 2023/07/24 10:10:54 by edechena         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/bsq.h"

bool	read_big_map(t_data *d, int fd, int buffer_size, int size)
{
	char		*buffer;
	long int	ret;

	buffer = malloc(sizeof(char) * buffer_size + 1);
	if (!buffer)
	{
		ft_putstr("Error\n");
		return (false);
	}
	ret = read(fd, buffer, buffer_size);
	buffer[ret] = '\0';
	while (ret != 0 && ret != -1)
	{
		d->map = ft_strjoin_bsq(d->map, size, buffer, buffer_size);
		if (!d->map)
		{
			ft_putstr("Error\n");
			return (false);
		}
		size += ret;
		ret = read(fd, buffer, buffer_size);
		buffer[ret] = '\0';
	}
	free(buffer);
	return (true);
}

bool	read_map(t_data *d, int fd, int buffer_size)
{
	long int	ret;

	d->map = malloc(sizeof(char) * BUFFER_INIT + 1);
	if (!d->map)
	{
		ft_putstr("Error\n");
		return (false);
	}
	ret = read(fd, d->map, BUFFER_INIT);
	d->map[ret] = '\0';
	if (ret != 0 && ret != -1)
		if (!read_big_map(d, fd, buffer_size, ret))
			return (false);
	return (true);
}

bool	read_stdin(t_data *d)
{
	if (!read_map(d, 0, BUFFER_STDIN))
		return (false);
	if (!check_map(d) || !read_len_lines(d))
		return (false);
	return (true);
}

bool	read_file(t_data *d, char *file)
{
	d->map = NULL;
	d->fd = open(file, O_RDONLY);
	if (d->fd < 0)
	{
		ft_putstr("Error\n");
		return (false);
	}
	if (!read_map(d, d->fd, BUFFER_SIZE))
	{
		close(d->fd);
		return (false);
	}
	close(d->fd);
	if (!check_map(d) || !read_len_lines(d))
		return (false);
	return (true);
}
